...
CREATE TABLE IF NOT EXISTS bank_connections (
  ...
  provider TEXT NOT NULL, -- 'truelayer', 'plaid', 'manual'
...
CREATE TABLE IF NOT EXISTS bank_transactions (
  ...
  external_transaction_id TEXT NOT NULL, -- Bank's transaction ID
  transaction_date DATE NOT NULL,
  description TEXT NOT NULL,
  amount DECIMAL(15,2) NOT NULL, -- Positive = credit, Negative = debit
  balance DECIMAL(15,2), -- Running balance after this transaction
  ...
  raw_data JSONB, -- Store full transaction data from bank
  created_at TIMESTAMPTZ DEFAULT NOW(),
...
-- Reconciliation discrepancies (items that don't match)
...
-- Reconciliation suggestions (AI recommendations for matching)
CREATE TABLE IF NOT EXISTS reconciliation_suggestions (
  ...
  match_reason TEXT NOT NULL, -- "Amount and date match exactly" / "Description similar"
...
BEGIN
  ...
  -- Find potential matches in trust transactions
  FOR trust_trans IN
    SELECT *
    FROM trust_transactions
  ...
  LOOP
    ...
    IF bank_trans.description ILIKE '%' || trust_trans.client_name || '%'
      OR trust_trans.description ILIKE '%' || SUBSTRING(bank_trans.reference FROM 1 FOR 10) || '%' THEN
      match_score := match_score + 0.2;
      reason := reason || '; Description similar';
    END IF;

    -- Only create suggestion if confidence > 0.6
...
BEGIN
  ...
  -- Get bank balance (latest transaction)
  ...
  -- Calculate client ledger balance
  client_bal := calculate_client_ledger_balance(account_id);

  -- Calculate office ledger balance (TODO: implement)
  office_bal := 0.0;
  ...
  -- SRA compliance check (bank balance should equal client ledger)
  ...
  SET
    ...
    matches_found = 0, -- Will be updated as matches are made
...
-- Trigger to auto-suggest matches when bank transaction is imported
CREATE OR REPLACE FUNCTION trigger_suggest_matches()
RETURNS TRIGGER AS $$
BEGIN
  -- Suggest matches for newly imported bank transaction
