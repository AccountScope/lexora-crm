...
CREATE TABLE IF NOT EXISTS client_portal_users (
  ...
  phone TEXT,
  password_hash TEXT NOT NULL, -- Separate auth from solicitors
...
-- Matter timeline events (for real-time client portal updates)
CREATE TABLE IF NOT EXISTS matter_timeline_events (
  ...
  created_at TIMESTAMPTZ DEFAULT NOW(),
  metadata JSONB, -- Store extra data (document_id, invoice_id, etc.)
  UNIQUE(matter_id, event_type, created_at)
);

-- Client notifications (push to portal)
CREATE TABLE IF NOT EXISTS client_notifications (
  ...
  title TEXT NOT NULL,
  body TEXT NOT NULL,
  action_url TEXT, -- Link to relevant page in portal
...
-- Matter milestones (for progress tracking)
...
-- Policies for client_portal_users (solicitors can manage, clients can view their own)
...
-- Policies for matter_timeline_events
...
-- Policies for client_notifications
...
-- Policies for matter_milestones
...
DROP TRIGGER IF EXISTS trigger_status_change_timeline ON cases;
