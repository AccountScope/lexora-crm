 ...
 * Replace the existing route.ts with this file after migration is complete.
 */

import { NextRequest } from "next/server";
import { requireUser } from "@/lib/auth";
import { getOrganizationContext } from "@/lib/api/tenant";
import { listCasesSecure, createCaseSecure } from "@/lib/api/cases-secure";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await listCasesSecure(context.organizationId, {
      search,
      status,
      limit: pageSize,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const created = await createCaseSecure(
